...
module SwaggerClient
  class DepartureBoardApi
    ...
    # @option opts [Integer] :direction stop id in order to get only departures of vehicles with a specified direction
    ...
    def get_departure_board(id, date, time, opts = {})
      data, _status_code, _headers = get_departure_board_with_http_info(id, date, time, opts)
      data
    end
    ...
    # @option opts [String] :format the required response format
    # @option opts [String] :jsonp_callback If JSONP response format is needed, you can append an additional parameter to specify the name of a callback function, and the JSON object will be wrapped by a function call with this name.
    # @return [Array<(DepartureBoard, Fixnum, Hash)>] DepartureBoard data, response status code and response headers
    def get_departure_board_with_http_info(id, date, time, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DepartureBoardApi.get_departure_board ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling DepartureBoardApi.get_departure_board"
      end
      # verify the required parameter 'date' is set
      if @api_client.config.client_side_validation && date.nil?
        fail ArgumentError, "Missing the required parameter 'date' when calling DepartureBoardApi.get_departure_board"
      end
      # verify the required parameter 'time' is set
      if @api_client.config.client_side_validation && time.nil?
        fail ArgumentError, "Missing the required parameter 'time' when calling DepartureBoardApi.get_departure_board"
      end
      # resource path
      local_var_path = '/departureBoard'
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'DepartureBoard')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DepartureBoardApi#get_departure_board\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
